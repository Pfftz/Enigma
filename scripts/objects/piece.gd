...
func _ready() -> void:
	...
	if !Engine.is_editor_hint():
		...
		if Global.is_piece_collected(room_name, piece_id):
			self.queue_free()
			return
...
func _on_body_entered(body: Node3D) -> void:
	if !Engine.is_editor_hint():
		...
		if body.is_in_group("player") or body.name.to_lower().contains("player"):
			# Try to collect the piece (returns false if already collected)
			if not Global.collect_piece(room_name, piece_id):
